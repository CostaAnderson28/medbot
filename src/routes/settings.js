...
router.get('/profile', (req, res) => {
  ...
  const db = getDb();
  const doc = db.prepare('SELECT id,name,clinic,email,instagram_handle,phone,whatsapp,address,bot_active,delay_first,delay_min,delay_max,prompt_kind,is_admin,tenant_type FROM doctors WHERE id=?').get(doctorId);
  db.close();
  res.json(doc);
});
...
router.put('/profile', (req, res) => {
  ...
  db.prepare("UPDATE doctors SET name=?,clinic=?,instagram_handle=?,phone=?,whatsapp=?,address=?,updated_at=datetime('now') WHERE id=?")
    .run(name, clinic, instagram_handle, phone, whatsapp, address, doctorId);
  db.close();
  res.json({ ok: true });
});
...
router.put('/delay', (req, res) => {
  ...
  db.prepare("UPDATE doctors SET delay_first=?,delay_min=?,delay_max=?,updated_at=datetime('now') WHERE id=?")
    .run(delay_first || 3, delay_min || 2, delay_max || 3, doctorId);
  db.close();
  res.json({ ok: true });
});
...
router.put('/toggle-bot', (req, res) => {
  ...
  if (!doctorId) return res.status(401).json({ error: 'Nao autenticado' });
  const db = getDb();
  db.prepare("UPDATE doctors SET bot_active=?,updated_at=datetime('now') WHERE id=?").run(active ? 1 : 0, doctorId);
  db.close();
